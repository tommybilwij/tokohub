...
async def commit_fp(pool, supplier_id, items, order_date=None, userid=None, shipping_cost=0, update_price=True, due_date=None, uraian=None):
    ...
    # ------------------------------------------------------------------
    # Phase 2 — Deferred stock updates with tanggal-based precedence.
    # For backdated FPs, skip updates if a newer transaction already set the price.
    # Precedence: tanggal DESC, noindex DESC.
    # ------------------------------------------------------------------
    inserted_lines = await execute_query(
        pool,
        "SELECT stockid, noindex FROM sthist WHERE becreff = %s AND tipetrans = 1",
        (fp_becreff,),
    )
    noindex_map = {r['stockid']: r['noindex'] for r in inserted_lines}

    from services.stock_restore import has_newer_hbeli_update, has_newer_hjual_update
    ...
    for line in preview['lines']:
        ...
        # hbeli + discounts + satbesar + packing: FP always sets, skip if newer FP exists
        try:
            if not await has_newer_hbeli_update(pool, artno, order_date, ni):
            ...
            logger.warning("Deferred stock hbeli update failed for %s", artno, exc_info=True)

        # hjual + bundling: only when update_price=True and no newer hjual update
        if update_price:
            try:
                if not await has_newer_hjual_update(pool, artno, order_date, ni):
                    await execute_modify(
                        pool,
                        """UPDATE stock
                ...
                logger.warning("Deferred bundling update failed for %s", artno, exc_info=True)
...
async def delete_fp(pool, fp_number: str) -> dict:
    ...
    # Get sthist lines for reversal (include tanggal for precedence checks)
    lines = await execute_query(
        pool,
        "SELECT stockid, qty, packing, qtybonus, noindex, tanggal FROM sthist WHERE becreff = %s AND tipetrans = 1",
        (becreff,),
    )

    # Build per-item noindex range + tanggal (needed for restore lookups after deletion)
    item_noindex = {}
    for line in lines:
        artno = line['stockid']
        ni = line['noindex']
    ...
    for artno, ni in item_noindex.items():
        ...
        if not await has_newer_hbeli_update(pool, artno, tgl, ni['max']):
            prev = await get_previous_hbeli(pool, artno, tgl, ni['min'])
            if not prev:
        ...
        if had_update_price and not await has_newer_hjual_update(pool, artno, tgl, ni['max']):
...
async def toggle_fp_update_price(pool, fp_number: str) -> dict:
    ...
    from services.stock_restore import (
        get_previous_hjual, has_newer_hjual_update,
        RESTORE_HJUAL_SQL, hjual_params,
    )

    # Get this FP's sthist lines (hjual + bundling + noindex + tanggal)
    fp_lines = await execute_query(
        pool,
        """SELECT stockid, noindex, tanggal,
                  hjual, hjual2, hjual3, hjual4, hjual5,
                  ispaketprc, over1, over2,
                  hjualo1, hjual2o1, hjual3o1, hjual4o1, hjual5o1,
                  hjualo2, hjual2o2, hjual3o2, hjual4o2, hjual5o2
    ...
    if new_val:
        ...
        for line in fp_lines:
            artno = line['stockid']
            if await has_newer_hjual_update(pool, artno, line['tanggal'], line['noindex']):
                continue
            try:
        ...
        # If no previous entry exists, keep current stock value (do NOT revert to init)
        for line in fp_lines:
            artno = line['stockid']
            if await has_newer_hjual_update(pool, artno, line['tanggal'], line['noindex']):
                continue
            prev = await get_previous_hjual(pool, artno, line['tanggal'], line['noindex'])
            if prev:
                try:
                    await execute_modify(pool, RESTORE_HJUAL_SQL, hjual_params(prev, artno))
                except Exception:
...
async def get_fp_comparison(pool, fp_number: str) -> dict | None:
    ...
    _COLS = """stockid, noindex, tanggal, artpabrik, artname,
...
async def update_fp(pool, fp_number, supplier_id, items, order_date=None, userid=None, shipping_cost=0, update_price=True, due_date=None, uraian=None):
    ...
    # Get old sthist lines with noindex + tanggal for in-place update
    ...
    updated_lines = await execute_query(
        pool,
        "SELECT stockid, noindex, tanggal FROM sthist WHERE becreff = %s AND tipetrans = 1",
        (fp_becreff,),
    )
    ...
    for line in preview['lines']:
        ...
        try:
            if update_price:
                if not await has_newer_hjual_update(pool, artno, tgl, ni):
                    await execute_modify(
                        pool,
                        """UPDATE stock
                           SET hjual = %s, hjual2 = %s, hjual3 = %s,
    ...
    # Fetch tanggal for removed items (they were deleted in Phase 1, use order_date as fallback)
    for artno, old in old_map.items():
        if artno in new_artnos:
            continue
        old_ni = old['noindex']
        old_tgl = old.get('tanggal') or order_date
        try:
            if not await has_newer_hbeli_update(pool, artno, old_tgl, old_ni):
                prev = await get_previous_hbeli(pool, artno, old_tgl, old_ni)
                if not prev:
                    prev = await get_init_hbeli(pool, artno)
                if prev:
                    await execute_modify(pool, RESTORE_HBELI_SQL, hbeli_params(prev, artno))
            if old_had_update_price and not await has_newer_hjual_update(pool, artno, old_tgl, old_ni):
